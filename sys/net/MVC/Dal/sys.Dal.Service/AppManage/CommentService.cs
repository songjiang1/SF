...
namespace sys.Dal.Service.AppManage
{
    ...
    public class CommentService : RepositoryFactory<CommentEntity>, ICommentService
    {
        ...
        public IEnumerable<CommentEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            if (!queryParam["VerifyMark"].IsEmpty())
            {
                string VerifyMark = queryParam["VerifyMark"].ToString();
                var Verify= VerifyMark.IsNull();
                expression = expression.And(t => t.VerifyMark == Verify);
            }
            if (!queryParam["Name"].IsEmpty())
            {
                string CreateUserName = queryParam["Name"].ToString();
                expression = expression.And(t => t.CreateUserName.Contains(CreateUserName));
            }
        ...
        {
            return this.BaseRepository().IQueryable().Where(t=> t.ObjectId==objId).OrderByDescending(t => t.CreateDate).ToList();
        }
        /// <summary>
        /// 公告实体
        /// </summary>
        ...
        public void SaveAudit(string keyValue, int audit, string verifyDescribe)
        {
            CommentEntity commentEntity = GetEntity(keyValue);
            commentEntity.Id = keyValue;
            commentEntity.VerifyMark = audit; 
            commentEntity.VerifyDescribe = verifyDescribe; 
            this.BaseRepository().Update(commentEntity);
        }
        #endregion
    }
}
