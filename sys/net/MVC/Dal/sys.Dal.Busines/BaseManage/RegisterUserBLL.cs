...
namespace sys.Dal.Busines.BaseManage
{
    ...
    public class RegisterUserBLL
    {
        ...
        /// <param name="userEntitys"></param>
        /// <returns></returns>
        public void ImportRegistUser(List<RegisterUserEntity> userEntitys)
        {
            try
            { 
                service.ImportRegistUser(userEntitys);
                CacheFactory.Cache().RemoveCache(cacheKey);  
            }
            catch (Exception)
            {
        ...
        public int UpdateHeadIcon(string keyValue, string HeadIcon)
        {
            ...
            {
                CacheFactory.Cache().RemoveCache(cacheKey);
                return service.UpdateHeadIcon(keyValue, HeadIcon);
            }
            catch (Exception)
            {
                return 0;
        ...
        public void UpadteParentId(string keyValue, string parentId, string placeholder, string oldUserId = "")
        {
            try
            {
                service.UpadteParentId(keyValue, parentId, placeholder, oldUserId);
                CacheFactory.Cache().RemoveCache(cacheKey);
            }
            catch (Exception)
            {
        ...
        public RegisterUserEntity CheckLogin(string username, string password)
        {
            ...
            if (userEntity != null && userEntity.Usource.ToLower().IndexOf("system")==-1 )
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <param name="description">个人简介内容</param>
        public void UpadteDescription(string keyValue, string description)
        {
            try
            {
                service.UpadteDescription(keyValue, description);
            }
            catch (Exception)
            {
                throw;
