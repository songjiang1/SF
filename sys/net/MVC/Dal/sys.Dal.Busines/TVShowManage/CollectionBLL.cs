...
namespace sys.Dal.Busines.TVShowManage
{
    ...
    public class CollectionBLL
    {
        ...
        public string cacheDtKey = "collectionDtCache";
        public string cacheUserKey = "collectionUserCache";
        public string cacheInitialsKey = "collectionInitialsCache";
        public string cacheListKey = "collectionListCache";
        #region 获取数据
        /// <summary>
        /// 用户列表
        /// </summary>
        ...
        {
            return service.GetInitialsUser(collection, OrderBy);
        }
        /// <summary>
        /// 用户首字母
        /// </summary>
        ...
        {
            return service.GetYiAnUser(collection, OrderBy);
        }
        ...
        {
            return service.GetFormEntity(queryJson);
        }
        ...
        {
            return service.GetList(collectionEntity);
        }
        /// <summary>
        /// 根据条件查询用户
        /// </summary>
        ...
        public void SaveForm(string keyValue, CollectionEntity collection)
        {
            try
            {
                ...
                {
                    if (collection.TVShowMenuId == "YiAn" && !string.IsNullOrWhiteSpace(collection.RepresentName))
                    {
                        if (Str.CheckStringChineseReg(collection.RepresentName))
                        {
                            collection.SimpleSpelling = Str.PinYin(collection.RepresentName).ToUpper();
                ...
                {
                    collection.Content = WebHelper.HtmlEncode(collection.Content);
                }
                if (!string.IsNullOrWhiteSpace(collection.Content1))
                {
                    collection.Content1 = WebHelper.HtmlEncode(collection.Content1);
                }
                if (!string.IsNullOrWhiteSpace(collection.Content2))
                {
                    collection.Content2 = WebHelper.HtmlEncode(collection.Content2);
                }
