...
namespace Client
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //var product1 = WebApiHelper.Get<ProductResultMsg>("http://localhost:63855/api/product/getproduct", parameters.Item1, parameters.Item2, staffId);
            Product product = new Product() { Id = 1, Name = "安慕希", Count = 10, Price = 58.8 };
            var product2 = WebApiHelper.Post<ProductResultMsg>("http://localhost:63855/api/product/addProduct", JsonConvert.SerializeObject(product), staffId);
            Console.Read();
        }
    }
}
