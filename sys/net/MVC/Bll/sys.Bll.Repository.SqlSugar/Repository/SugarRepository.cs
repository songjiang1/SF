...
namespace sys.Bll.Repository.SqlSugar
{
    ...
    public class SugarRepository : ISugarRepository
    {
        ...
        public SimpleClient simpleClient { get { return new SimpleClient(DbContext); } }//用来处理表的常用操作
        ...
        public ISugarRepository BeginTrans()
        {
        ...
        public List<TResult> QueryMuchDescriptor<T, T2, TResult>(
            Expression<Func<T, T2, object[]>> joinExpression,
            Expression<Func<T, T2, TResult>> selectExpression,
            Pagination query, out int totalCount) where T : class, new()
        {
        ...
        /// <returns>实体</returns>
        public List<T> QueryList<T>(Pagination query = null) where T : class, new()
        {
            ISugarQueryable<T> up = DbContext.Queryable<T>().With(SqlWith.NoLock);
            if (query != null)
            {
        ...
        /// <param name="totalCount">总行数</param>
        /// <returns>实体</returns>
        public List<T> QueryPageList<T>(Pagination query, out int totalCount, Expression<Func<T, bool>> whereLambda = null) where T : class, new()
        {
            if (query == null)
            {
        ...
        /// <param name="totalCount">总行数</param>
        /// <returns>实体</returns>
        public List<T> QueryPageList<T>(Pagination query, out int totalCount) where T : class, new()
        {
            if (query == null)
            {
                throw new ArgumentNullException(nameof(query));
        ...
        /// <returns></returns>
        public T GetEnty<T>(string Key) where T : class, new()
        {
            var datas = DbContext.Queryable<T>().With(SqlWith.NoLock).InSingle(Key);
            return datas;
        }
        /// <summary>
        /// 查询实体
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        public T GetEnty<T>(T t) where T : class, new()
        {
            var datas =  simpleClient.GetById<T>(t) ;
            return datas;
        }
        ...
        public DataTable QueryDataTablePageList<T>(Pagination query, out int totalCount) where T : class, new()
        {
            if (query == null)
            {
                throw new ArgumentNullException(nameof(query));
        ...
        /// <param name="query">过滤条件</param>
        /// <returns></returns>
        public List<T> QueryMapper<T>(Action<T> mapperAction, Pagination query = null) where T : class, new()
        {
