...
function checkedChanged() {
    ...
    switch (name) {
        ...
        case "useChartAnimation":
            ...
            break;
        case "checkboxLockCell":
            setSelectedCellsLock(getCheckValue("checkboxLockCell"));
            break;
...
function syncSheetPropertyValues() {
    ...
    if (!$(sheet).data("bind")) {
        ...
        sheet.bind(spreadNS.Events.ShapeRemoved, function (event, args) {
            // check special type items and switch to cell tab (laze process)
            if (isSpecialTabSelected()) {
                onCellSelected();
            }
...
function itemSelected() {
    ...
    switch (name) {
        ...
        case "spreadTheme":
            ...
            break;
        case "fillCustom":
            processChangeCustomList(dataValue);
            break;
...
function setDropDownList(container, list) {
    var root = $("#" + $(container).data("list-ref")), str = "";
    $(".menu-item", root).unbind();
    root.empty();
    list.forEach(function(item){
        str += '<div class="menu-item"><div class="image fa fa-check"></div><div class="text localize" data-value="'+item.value+'">'+item.name+'</div><div class="shortcut"></div></div>';
    });
    root.append(str);
    $(".menu-item", root).click(itemSelected);
}
...
function divButtonClicked() {
    ...
    switch (id) {
        ...
        case "unGroupShape":
            ...
            break;
        case "fillCustomListAdd":
            addCustomList();
            break;
        case "fillCustomListDelete":
            deleteCustomList();
            break;
...
function doPrepareWork() {
    ...
    refreshDragFillCustomList();                                    // Fill custom list
...
function setCellContent() {
    ...
    sheet.getCell(row, ++col).value("ABCDEFG HIJKLMN OPQ-RST UVW-XYZ").wordWrap(true);
    ...
    spread.bind(spreadNS.Events.ActiveSheetChanged, function (event, args) {
        ...
        if(newSheet.name() === 'Chart'){
            ...
            if(isFirstChart){
                ...
                var lineType = GC.Spread.Sheets.Charts.ChartType.lineMarkers;
...
// spread fill custom list related items
function processChangeCustomList(index) {
    var viewTxt = $("#fillCustomListView");
    var addBtn = $("#fillCustomListAdd");
    var delBtn = $("#fillCustomListDelete");
    var str = "", list = [];
    spread._selectCustomListIndex = index;
    if (index < 0) {
        list = spread._fillBuildinList[-1-index];
        ...
        list = spread._fillCustomList[index-1];
        viewTxt.attr("disabled", false);
        addBtn.attr("disabled",false);
...
function addCustomList() {
    ...
    for (var i = 0; i < spread._fillBuildinList.length; i++) {
        if(str === spread._fillBuildinList[i].toString()){ //check the list wether have same list
            return;
        }
    }
    for (var i = 0; i < spread._fillCustomList.length; i++) {
        if(str === spread._fillCustomList[i].toString()){
            return;
        }
    }

    var index = spread._selectCustomListIndex;
    if(index === 0){
        spread._fillCustomList.push(arr);
        spread.options.customList = spread._fillCustomList;
    } else if (index > 0) {
        spread._fillCustomList[index - 1] = arr;
        spread.options.customList = spread._fillCustomList;
    }
...
function deleteCustomList(){
    if(spread._selectCustomListIndex > 0){
        var index = spread._selectCustomListIndex;
        if (index > 0) {
            spread._fillCustomList.splice(index - 1, 1);
            spread.options.customList = spread._fillCustomList;
        }
        refreshDragFillCustomList();
    }
...
function refreshDragFillCustomList() {
    ...
    spread._fillBuildinList = buildinlist;
    var list = [];
    
    list.push({name:getResource("spreadTab.customList.newList"), value:0});
    for (var i = 0; i < buildinlist.length; i++) {
        list.push({name: buildinlist[i][0] + "," + buildinlist[i][1] + "," + buildinlist[i][2] + "...", value:(-1 - i)});
    }

    var customList = spread.options.customList || [];
    spread._fillCustomList = customList;
    for (var i = 0; i < customList.length; i++) {
        list.push({name:customList[i].toString(), value:(i + 1)});
    }
    spread._selectCustomListIndex = 0;
    setDropDownList("#fillCustomLists", list);
    setDropDownValueByIndex("#fillCustomLists", 0);
...
// spread fill custom list related items (end)
...
function isChartHasMarker(chart, seriesIndex) {
    var series = chart.series().get(seriesIndex)
    var type = series.chartType;
    return type === 26 /**Line with Markers */ || type === 27 /**Stacked Line with Markers */ || type === 28 /**100% Stacked Line with Markers */ 
        || type === 33 /**Scatter with Smoothed Lines */ || type === 35 /**Scatter with Lines. */ || type === 42 /**Radar with data makers */;
}
function updateChartSeriesSetting(chart,seriesIndex) {
    ...
    if(isChartHasMarker(chart, seriesIndex)){
        $("#seriesSymbolGroup").show();
        setDropDownValue('chartSymbolShapeValue',seriesItem.symbol.shape);
        setTextValue('chartSymbolSize',seriesItem.symbol.size);
        var fillColor = getColorByThemeColor(seriesItem.symbol.fill);
        setColorValue("chartSymbolColor", getRGBAColor(fillColor, 1 - seriesItem.symbol.fillColorTransparency));
        setDropDownValue('chartSymbolBorderLineTypeValue',seriesItem.symbol.border.lineType);
        setTextValue('chartSymbolBorderWidth',seriesItem.symbol.border.width);
        var borderColor = getColorByThemeColor(seriesItem.symbol.border.color);
        setColorValue("chartSymbolBorderColor", getRGBAColor(borderColor, 1 - seriesItem.symbol.border.colorTransparency));
    } else {
        $("#seriesSymbolGroup").hide();
    }
}
...
function applyChartSeries() {
    ...
    if(chart){
        ...
        seriesItem.backColor = getBackgroundColor('chartSeriesColor');
        seriesItem.axisGroup  = getDropDownValue("chartSeriesGroupValue");
        seriesItem.border.width = parseInt(getTextValue('chartSeriesLineWidth'));
        seriesItem.border.color = getBackgroundColor('chartSeriesLineColor');
        seriesItem.border.transparency = getColorTransparency("chartSeriesLineColor");
        seriesItem.backColorTransparency = getColorTransparency("chartSeriesColor");
        if(isChartHasMarker(chart, seriesIndex)){
            seriesItem.symbol.shape = getDropDownValue('chartSymbolShapeValue');
            seriesItem.symbol.size = parseInt(getTextValue('chartSymbolSize'));
            seriesItem.symbol.fill = getBackgroundColor('chartSymbolColor');
            seriesItem.symbol.fillColorTransparency = getColorTransparency("chartSymbolColor");
            seriesItem.symbol.border.lineType = getDropDownValue('chartSymbolBorderLineTypeValue');
            seriesItem.symbol.border.width = parseInt(getTextValue('chartSymbolBorderWidth'));
            seriesItem.symbol.border.color = getBackgroundColor('chartSymbolBorderColor');
            seriesItem.symbol.border.colorTransparency = getColorTransparency("chartSymbolBorderColor");
        }
