...
namespace sys.Util.WebControl
{
    ...
    public static class QueryTree
    {
        ...
        /// <param name="parentId"></param>
        /// <returns></returns>
        public static List<T> TreeBranchWhere<T>(this List<T> entityList, Predicate<T> condition, string primaryKey, string parentId = "ParentId") where T : class
        {
            List<T> allList = entityList;
            List<T> locateList = entityList.FindAll(condition);
            var parameter = Expression.Parameter(typeof(T), "t");
            //模糊查询表达式
            List<T> treeList = new List<T>();
            ...
            foreach (T entity in allList)
            { 
                //向下查询
                string pId = entity.GetType().GetProperty(primaryKey).GetValue(entity, null).ToString();
                ...
                while (true)
                {
                    ...
                    Predicate<T> upLambda = (Expression.Equal(parameter.Property(parentId), Expression.Constant(pId))).ToLambda<Predicate<T>>(parameter).Compile();
                    T upRecord = entityList.Find(upLambda);
                    if (upRecord != null)
                    {
                        treeList.Add(upRecord);
                        pId = upRecord.GetType().GetProperty(primaryKey).GetValue(upRecord, null).ToString();
                    }
                    else
                    {
                        break;
